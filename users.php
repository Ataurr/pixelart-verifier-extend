...
function px_verify_show_extra_profile_fields($user) {
    ...
    else if (isset($_GET['action']) && $_GET['action'] == 'upgradeCwv') {
        $result = px_verify_upgrade_cwv($user_id);
    } else {
        $result = '';
    }
    ?>
    ...
    <table class="form-table">
        ...
        <tr>
            ...
            <td>
                ...
                <!--ar-->
                <a href="?user_id=<?php echo $user_id; ?>&action=upgradeCwv" class="button" >Upgrade from CloudWork Verifi</a>
                <?php echo $result; ?>
            </td>
        </tr>
    </table>
...
// Upgrade from the CloudWork Verifi

//ar
function px_verify_upgrade_cwv($user_id) {
    
        $data = get_user_meta($user_id, '_cw_purchase_code', true);
        ...
        if (!is_wp_error($verify)) {
            ...
            return '<div id="message" class="updated notice is-dismissible"><p><strong>Upgraded the data from Cloud Work Verifi!</strong></p></div>';
        } else {
            $error_string = $verify->get_error_message();
            return '<div id="message" class="error notice is-dismissible"><p>' . $error_string . '</p></div>';
        }
...
function px_verifier_bulk_upgrade_footer() {
    ...
    <script type="text/javascript">
        jQuery(document).ready(function ($) {
            ...
            $('<option>').val('upgrade_cwv').text('Upgrade from Cloud Work verifier').appendTo("select[name='action'], select[name='action2']");
        });
    </script>
    <?php
}
...
function px_verifier_bulk_upgrade_cwv() {
    $users = get_users();
    $refreshed = array();

    foreach ($users as $user) {
        $id = $user->id;
        $result = px_verify_upgrade_cwv($id);
        if ($result == '<div id="message" class="updated notice is-dismissible"><p><strong>Upgraded the data from Aqua Verifier!</strong></p></div>') {
            $refreshed[$id] = 'upgraded';
        } else {
            $result = str_replace('<div id="message" class="error notice is-dismissible"><p>', '', $result);
...
function px_verifier_bulk_notice() {
    ...
    add_action('admin_action_upgrade_cwv', 'px_verifier_bulk_upgrade_cwv');
